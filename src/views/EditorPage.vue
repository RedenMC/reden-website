...
export default {
  name: 'CodeEditor',
  mounted() {
    workspace = Blockly.inject('redenBlockly', {
      toolbox: toolbox,
      scrollbars: true,
      horizontalLayout: false,
      toolboxPosition: 'start',
    });
    const data = localStorage.getItem('reden-workspace');
    if (data != null) {
      Blockly.serialization.workspaces.load(workspace, JSON.parse(data));
    }
    setInterval(() => {
      this.code = javascriptGenerator.workspaceToCode(workspace);
      this.elements = JSON.stringify(
        Blockly.serialization.workspaces.save(workspace),
      );
      localStorage.setItem('reden-workspace', this.elements);
    }, 1000);
  },
  data() {
    return {
...
<template>
  <div id="redenBlockly" style="height: 600px; width: 100%"></div>
  <div id="code">
    <pre>
      {{ code }}
    </pre>
